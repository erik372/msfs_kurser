...
$$V(x,y) = TR - TC = 10x + 20y - (x^{2} + 2y^{2} + 10) \tag{4}$$
...
$$\max_{m.\ h.t\ \ x,y}{V(x,y)}\ = 10x + 20y - (x^{2} + 2y^{2} + 10) \tag{5}$$
...
Givet priserna $p_{x} = 10$ och $p_{y} = 20$ och nuvarande produktionskostnader kan företaget maximera sin vinst genom att producera 5 enheter av produkt A och 5 enheter av produkt B, och gör då en vinst på 65 kr. Företagets totala kostnader ges av ekvation 3. Tar vi förstaderivatan av totala kostnader med hänsyn till x respektive y får vi företagets *marginalkostnader*:
