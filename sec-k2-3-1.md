...
\begin{align}
\sum_{i}^{}\widehat{{V_{i}}^{2}} &= \sum_{i}^{}\left( Y_{i} - \widehat{Y_{i}} \right)^{2} \tag{2}\\
&= (3 - 3{,}5)^{2} + (4 - 3{,}5)^{2} + (5 - 3{,}5)^{2} + (6 - 3{,}5)^{2} \\
&= 9
\end{align}
...
För observation 2 och 3 har vi att $\left( Y_{2} = 2 \right)$ och $\left( Y_{3} = 5 \right)$: \begin{align}
\sum_{i}^{}{\widehat{V}}_{i}^{2} &= \sum_{i}^{}\left( Y_{i} - \widehat{Y_{i}} \right)^{2} \tag{7}\\
&= \left( Y_{1} - \widehat{Y}_{1} \right)^{2} + \left( Y_{2} - \widehat{Y}_{2} \right)^{2} + \left( Y_{3} - \widehat{Y}_{3} \right)^{2} + \left( Y_{4} - \widehat{Y}_{4} \right)^{2} \\
&= 0 + \left( 2 - \frac{13}{4} \right)^{2} + \left( 5 - \frac{15}{4} \right)^{2} + 0 \\
&= 3{,}1875 > 2{,}5
\end{align}
...
::: {.fig-caption}
Beskrivning: Diagrammet visar tre linjer. Linje A är regressionslinjen, beräknad med minstakvadratmetoden. Linje B är horisontell och linje C går igenom två av punkterna. Av de tre linjerna har linje A det minstas summerade vertikala avståndet mellan linjen och punkterna.
:::
